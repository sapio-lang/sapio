...
pub enum EmulatorError {
    ...
    InvalidResponse,
    /// A federated signer threshold is outside the policy's valid range.
    InvalidThreshold(sapio_base::miniscript::ThresholdError),
}
...
impl From<bitcoin::bip32::Error> for EmulatorError {
    fn from(e: bitcoin::bip32::Error) -> EmulatorError {
...
pub fn sign_checked(emulator: &dyn CTVEmulator, request: Psbt) -> Result<Psbt, EmulatorError> {
...
pub fn validate_signing_response(request: &Psbt, response: &Psbt) -> Result<(), EmulatorError> {
