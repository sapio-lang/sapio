...
| [Eltoo with WASM fragments](ELTOO_FRAGMENTS.md) | Executable TemplateHash/IKEY/CSFS update and settlement logic. One joint authorization rebinds against an older state, with proof recovery from its published transaction and a replaceable fee sponsor. | Funding has no settlement path; native CLTV orders updates; state-specific hashes and CSV protect settlement. Unit tests cover replay and proof recovery; an isolated Core scenario confirms a stale-state override and the restarted contest window. |
