...
| [jamesob-vault](../plugin-example/jamesob-vault) | Triggered withdrawal and recovery with optional CPFP outputs. Fee rates are satoshis per 1,000 weight units and round an unsigned-size estimate up; witness growth is outside that estimate. Native tests check rounding, overflow and actual vault funding; the catalog checks both transitions. |
