...
from typing import (Dict, Generic, List, Literal, Optional, Protocol, Tuple,
                    Type, TypeVar, Union)
...
from sapio.contract.decorators import (check, enable_if, guarantee, require,
                                       unlock, unlock_but_suggest)
from sapio.contract.txtemplate import TransactionTemplate
from sapio.examples.p2pk import PayToPubKey, PayToSegwitAddress
from sapio.script.clause import (Clause, PreImageCheckClause, RelativeTimeSpec,
    ...
    class BinaryBet(Contract):
        ...
        if t1 is PubKey:
            ...
            def pay_hi(self):
                ...
                tx.add_output(self.amount.assigned_value, self.hi_outcome.assigned_value)
                return tx
        ...
        if t2 is PubKey:
            ...
            def pay_lo(self):
                tx = TransactionTemplate()
                tx.add_output(self.amount.assigned_value, self.lo_outcome.assigned_value)
                return tx
    return BinaryBet
    
a = BinaryBetFactory(PubKey, Contract)
b = BinaryBetFactory(Contract, Contract)
c = BinaryBetFactory(PubKey, PubKey)
d = BinaryBetFactory(Contract, PubKey)
BinaryBetContract = Union[a,b,c,d]
T = Union[PubKey, Contract]
...
class PriceOracle:    
    class BetStructure:
        ...
        def __init__(self, l:List[Tuple[int, Tuple[Hash, Hash], Contract]]):
            self.price_array = l
        @classmethod
        def from_json_data(cls, data: List[Tuple[int, Tuple[Hash, Hash], Tuple[Amount, str]]], ctx):
            pass
    ...
    def generate(
        ...
        if len(price_array) > 1:
            # TODO: Sort on first entry...
            if not is_sorted:
                if any(price_array[i][0] < price_array[i+1][0] for i in range(len(price_array)-1)):
                    price_array.sort()
            ...
            lo_outcome = PriceOracle.generate(PriceOracle.BetStructure(price_array[middle:]), amount, True)
            hi_outcome = PriceOracle.generate(PriceOracle.BetStructure(price_array[:middle]), amount, True)
            return b(price=price, hi_outcome=hi_outcome, lo_outcome=lo_outcome, h_price_hi = h_hi, h_price_lo=h_lo, amount=amount)
        else:
