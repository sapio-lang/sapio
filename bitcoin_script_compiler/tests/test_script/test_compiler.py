...
class TestCompiler(unittest.TestCase):
    def test_clause_to_cnf(self):
        clauses = [
            [
                AfterClause(AssignedVariable(AbsoluteTimeSpec.at_height(n), str(n)))
        ...
        self.assertSetEqual(
            frozenset(x.a.assigned_value.time for y in output for x in y),
            frozenset(range(0, 100 ** 2)),
            "does not preserve values",
        )
        self.assertSetEqual(
            frozenset(frozenset(x.a.assigned_value.time for x in y) for y in output),
    ...
    def test_clause_to_cnf_random(self):
        ...
        to_set = lambda s: frozenset(
            frozenset(y.a.assigned_value.time for y in x) for x in s
        )
