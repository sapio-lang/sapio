...
from .clause import Clause, SignatureCheckClause, PreImageCheckClause, \
...
class FragmentCompiler:
    ...
    @_compile.register
    def _compile_and(self, arg: SignatureCheckClause, witness:WitnessTemplate) -> CScript:
        if TYPE_CHECKING:
            assert callable(self._compile)
        variable : CScript = self._compile(UnassignedVariable(b"_signature_by_"+arg.a.name), witness)
        script: CScript = self._compile(arg.a, witness) + CScript([AllowedOp.OP_CHECKSIGVERIFY])
        assert len(variable) == 0
        return script

    @_compile.register
    def _compile_preimage(self, arg: PreImageCheckClause, witness: WitnessTemplate) -> CScript:
        if TYPE_CHECKING:
            assert callable(self._compile)
        variable: CScript = self._compile(UnassignedVariable(b"_preimage_of_"+arg.a.name), witness)
        script: CScript = CScript([AllowedOp.OP_SHA256]) + self._compile(arg.a, witness) + CScript([AllowedOp.OP_EQUALVERIFY])
        assert len(variable) == 0
        return script
