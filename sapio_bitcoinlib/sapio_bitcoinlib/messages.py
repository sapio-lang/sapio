...
class CTxIn:
    ...
    def __init__(
        ...
        scriptSig: CScript = CScript(),
        nSequence: Sequence = Sequence(0),
    ) -> None:
        if outpoint is None:
...
class CTxOut:
    ...
    def __init__(
        self, nValue: Amount = Amount(int64(0)), scriptPubKey: CScript = CScript()
    ) -> None:
        self.nValue = nValue
...
class CBlockHeader:
    ...
    def __init__(self, header=None):
        if header is None:
            ...
            self.sha256 = sapio_bitcoinlib.hash_functions.sha256
...
class CBlock(CBlockHeader):
    ...
    def calc_merkle_root(self):
        ...
        for tx in self.vtx:
            tx.calc_sha256()
            hashes.append(ser_uint256(sapio_bitcoinlib.hash_functions.sha256))
    ...
    def initialize_from_block(
        ...
        for i in range(len(block.vtx)):
            if i not in prefill_list:
                tx_hash = sapio_bitcoinlib.hash_functions.sha256
                if use_witness:
